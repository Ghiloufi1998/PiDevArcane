...
private void Chercher (ActionEvent event) throws SQLException {
    ...
    Img.setFitHeight(99); //726
    Img.setFitWidth(99); //500
    ...
    private void Ajouter_Transport(ActionEvent event) throws SQLException {
        ...
        if (controleTextFieldNonNumerique(DescriptionField) ||  controleTextFieldNonNumerique(TypeField)||  controleTextFieldNumerique(PriceField)||  controleTextFieldNumerique1(DisponibiliteField)); else 
        {
...
Hebergement k4=sh.get(Table_transport.getSelectionModel().getSelectedItem().getHebergement().getDescription());
 ...
 @FXML
    ...
    private void CodeQr(ActionEvent event) {
        ...
        String td= t.getDescription()+t.getType()+t.getPrix()+t.toString();
            ByteArrayOutputStream out = QRCode.from(td)
                .to(ImageType.PNG).stream();
        
        //String Path_name = new File("/").getAbsolutePath();
        try {
            FileOutputStream fout = new FileOutputStream(new File("C:/Users/Ghiloufi/Desktop/Mostfa1.PNG"));
