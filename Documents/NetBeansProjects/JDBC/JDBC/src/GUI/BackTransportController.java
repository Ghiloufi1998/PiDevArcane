...
private void Chercher (ActionEvent event) throws SQLException {
    ...
    private void Ajouter_Transport(ActionEvent event) throws SQLException {
        ...
        if (controleTextFieldNonNumerique(DescriptionField) ||  controleTextFieldNonNumerique(TypeField)); else 
        {
        ...
        Hebergement k4=sh.get(heb.getText());
            Transport t = new Transport(TypeField.getText(), DescriptionField.getText(),id, id2,filePath,k4);
            if((ts.nbrv(t)==0))
            {ts.ajout(t);
